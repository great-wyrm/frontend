  ...
  return (
    ...
    >
      <Flex gap='10px'>
        ...
        <Text p='5px 10px' fontSize='10px' borderRadius='10px' border='1px solid white'>
          {stage === currentStage ? `Active stage - ${stageMetadata.title}` : `Stage ${stage} - ${stageMetadata.title}`}
        </Text>
        {stage < currentStage && stage <= numberOfStages ? (
          <IconButton
            aria-label='forward'
            size='10px'
            _hover={{ bg: '#2d2d2d' }}
      ...
      <Flex
        ...
        borderColor={
          userDidVote &&
          votes.data &&
          votes.data[selectedPath - 1] &&
          votes.data[selectedPath - 1] === Math.max(...votes.data.filter((n) => n))
            ? '#6DD08E'
            : 'white'
        }
      >
        <Text textAlign='center' fontWeight='700' fontSize='14px'>
          Path {selectedPath} <br /> {stageMetadata.paths[selectedPath - 1].title}
        </Text>
