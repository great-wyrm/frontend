...
import useMoonToast from '../components/useMoonToast'
...
import { ChoosePathData, SessionMetadata } from '../../src/components/GoFPTypes'
...
export const useGofpContract = ({
  ...
  const tokenContract = new web3ctx.web3.eth.Contract(ERC721MetadataABI) as unknown as MockERC721
  ...
  const currentStage = useQuery<number>(
    ...
    async () => {
      if (gardenContractAddress == ZERO_ADDRESS || sessionId < 1 || !sessionInfo.data) return 1 //TODO

      const result = await gardenContract.methods.getCurrentStage(sessionId).call()
  ...
  const correctPaths = useQuery<number[]>(
    ...
    async () => {
      ...
      for (let i = 1; i < currentStage.data; i++) {
        const ans = await gardenContract.methods.getCorrectPathForStage(sessionId, i).call()
  ...
  function useTokenUris(tokenIds: number[]) {
    return useQuery(['tokens_uri', tokenIds], () => getTokensUri(tokenIds), {
      ...hookCommon,
  ...
  const ownedTokens = useQuery<number[]>(
    ...
    async () => {
      tokenContract.options.address = sessionInfo.data[0]
      const balance = await tokenContract.methods.balanceOf(web3ctx.account).call()
      const tokens = []

      for (let i = 0; i < parseInt(balance); i++) {
        const tok = await tokenContract.methods.tokenOfOwnerByIndex(web3ctx.account, i).call()
  ...
  const stakeTokens = useMutation(
    (tokenIds: number[]) => {
      console.log('Attempting to stake ', tokenIds, ' into session ', sessionId, '.')
      const gardenContract: any = new web3ctx.web3.eth.Contract(GardenABI) as any as GardenABIType
      gardenContract.options.address = gardenContractAddress
      return gardenContract.methods.stakeTokensIntoSession(sessionId, tokenIds).send({
  ...
  const unstakeTokens = useMutation(
    (tokenIds: number[]) => {
      return gardenContract.methods.unstakeTokensFromSession(sessionId, tokenIds).send({
        from: web3ctx.account,
      })
    },
    {
  ...
  const stakedTokens = useQuery<number[]>(
    ...
    async () => {
      const balance = await gardenContract.methods.numTokensStakedIntoSession(sessionId, web3ctx.account).call()
      const tokens = []
      for (let i = 1; i <= parseInt(balance); i++) {
        const tok = await gardenContract.methods.tokenOfStakerInSessionByIndex(sessionId, web3ctx.account, i).call()
        tokens.push(parseInt(tok))
        // TODO MULTICALL
      }
